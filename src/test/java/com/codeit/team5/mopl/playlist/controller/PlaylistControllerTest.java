...
class PlaylistControllerTest {
    ...
    void update() throws Exception {
        ...
        // when & then
        mockMvc.perform(patch("/api/playlists/{id}", playlistId).with(csrf()).principal(principal)
                .contentType(MediaType.APPLICATION_JSON)
                .content(objectMapper.writeValueAsString(request))).andDo(print())
    ...
    void deletePlaylist() throws Exception {
        ...
        // when & then
        mockMvc.perform(delete("/api/playlists/{id}", playlistId).with(csrf()).principal(principal))
    ...
    void addContent() throws Exception {
        // given
        willDoNothing().given(playlistService).addContent("user@test.com", playlistId, contentId);

        // when & then
        mockMvc.perform(
                post("/api/playlists/{playlistId}/contents/{contentId}", playlistId, contentId)
                        .with(csrf()).principal(principal))
                .andDo(print()).andExpect(status().isNoContent());
    }
