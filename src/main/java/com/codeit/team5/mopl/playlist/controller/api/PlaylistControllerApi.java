...
public interface PlaylistControllerApi {
    ...
    ResponseEntity<PlaylistResponse> find(@PathVariable UUID id);

    @Operation(summary = "플레이리스트 목록 조회 (커서 페이지네이션)")
    ResponseEntity<CursorResponse<PlaylistResponse>> findCursor(@ModelAttribute PlaylistCursorRequest request);

    @Operation(summary = "플레이리스트 생성", description = "생성한 플레이리스트는 API 요청자 본인의 플레이리스트로 생성됩니다.")
    ResponseEntity<PlaylistResponse> create(Principal principal, @RequestBody PlaylistCreateRequest request);

    @Operation(summary = "플레이리스트 수정", description = "플레이리스트 소유자만 수정할 수 있습니다.")
    ResponseEntity<PlaylistResponse> update(Principal principal, @PathVariable UUID id, @RequestBody PlaylistUpdateRequest request);
    ...
    ResponseEntity<Void> addContent(Principal principal, @PathVariable UUID playlistId, @PathVariable UUID contentId);

    @Operation(summary = "플레이리스트에서 콘텐츠 삭제", description = "플레이리스트 소유자만 콘텐츠를 삭제할 수 있습니다.")
    ResponseEntity<Void> removeContent(Principal principal, @PathVariable UUID playlistId, @PathVariable UUID contentId);
}
